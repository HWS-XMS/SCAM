...
class TestTrace(unittest.TestCase):
    ...
    def test_trace_creation(self):
        ...
        trace = Trace(samples=np.array([1, 2, 3], dtype=np.float32))
        np.testing.assert_array_equal(trace.samples, [1, 2, 3])
        self.assertIsNone(trace.timestamp)
...
class TestSeries(unittest.TestCase):
    ...
    def test_series_writing(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(10):
                trace = Trace(samples=np.array([i, i+1, i+2], dtype=np.float32))
                self.series.add_trace(trace)
...
class TestTraceDB(unittest.TestCase):
    ...
    def test_hdf5_save_load(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(5):
                trace = Trace(
                    samples=np.array([i, i+1, i+2], dtype=np.float32),
                    stimulus=f"input_{i}",
            ...
            np.testing.assert_array_almost_equal(loaded_trace.samples, np.array([0, 1, 2], dtype=np.float32))
...
class TestIntegration(unittest.TestCase):
    ...
    def test_complete_workflow(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(20):
                measurement = np.random.random(1000).astype(np.float32)
                trace = Trace(
                    samples=measurement,
