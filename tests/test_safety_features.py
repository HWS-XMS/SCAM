#!/usr/bin/env python3
"""
Tests for safety features in TraceDB.save_hdf5()
"""
...
class TestSafetyFeatures(unittest.TestCase):
    ...
    def test_duplicate_series_warning(self):
        ...
        db_loaded = TraceDB.load_hdf5(self.test_file)
        exp = db_loaded.experiments["Exp1"]
        self.assertEqual(len(exp.series), 1)
    ...
    def test_data_integrity_after_merge(self):
        ...
        for i in range(3):
            trace = Trace(
                samples=np.ones(10, dtype=np.float32) * i,
                stimulus=f"stim1_{i}",
                key=f"key1_{i}",
            )
        ...
        for i in range(3):
            trace = Trace(
                samples=np.ones(10, dtype=np.float32) * (i + 10),
                stimulus=f"stim2_{i}",
                key=f"key2_{i}",
            )
        ...
        for i, trace in enumerate(series1_loaded):
            np.testing.assert_array_almost_equal(trace.samples, np.ones(10, dtype=np.float32) * i)
        ...
        for i, trace in enumerate(series2_loaded):
            np.testing.assert_array_almost_equal(trace.samples, np.ones(10, dtype=np.float32) * (i + 10))
